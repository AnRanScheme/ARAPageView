...
//MARK:delegate 1
protocol MagiTitleViewDelegate:class {//1 NSObjectProtocol 2 class 只能被类遵守
    func titleView(_ titleView :MagiTitleView,targetIndex:Int)
}

class MagiTitleView: UIView {
    
    //MARK:delegate 2
    weak var delegate : MagiTitleViewDelegate?
    ...
    init(frame: CGRect,titles : [String],style : MagiPageStyle) {
        self.titles = titles
...
//MARK:- 代理的使用 2
extension MagiTitleView: MagiContentViewDelegate{
    func contentView(_ contentView: MagiContentView, didEndScroll inIndex: Int) {
        currentIndex = inIndex
        //调整中间位置offset
        adjustLabelPos()
    }
    
    func contentView(_ contentView: MagiContentView, sourceIndex: Int, targetIndex: Int, progress: CGFloat) {
