...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let titles = ["推荐","视频"]//,"图片","段子","投票"]
        let style = MagiPageStyle()
        style.isNeedTitleScale = false//缩放支持
        style.bottomLineColor = UIColor.blue
        style.normalColor = UIColor.black
        style.selectColor = UIColor.blue
        style.bottomLineExtendWidth = 50//相当于inset，因为transform会影响frame，用bottomLineExtendWidth改变底部跟踪滚动条（下划线）的缩进
        ...
        let pageView = MagiPageView(frame: pageViewFrame, titles: titles, titleStyle: style, childVCs: childVCs, parentVC: self)
